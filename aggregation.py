...
- Mid-to-late transformer layers (~60-95% depth) encode factuality signals
...
- Mean-pooling over real (non-padding) tokens is more stable than the
...
"""Selection rationale: ~50%, ~66%, ~83%, ~100% depth — a small set
...
def aggregate(
    ...
    # Ensure real_idx is on the same device as hidden_states
...
def extract_geometric_features(
    ...
    """
    real_idx = _real_token_indices(attention_mask)
    # Fix: move real_idx to the same device as hidden_states
    real_idx = real_idx.to(hidden_states.device)
    ...
    seq_len_feat = torch.tensor(
        [len(real_idx) / 100.0], dtype=mean_norms.dtype, device=hidden_states.device
    )
    return torch.cat(
        [
            mean_norms,      # (n_layers,)
